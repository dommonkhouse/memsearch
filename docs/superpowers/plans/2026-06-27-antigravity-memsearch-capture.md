...
- Program arguments include `source-sync antigravity` and do not include `--index`; daily capture writes and scans cards, while canonical indexing remains a manual review-gated step.
...
```bash
uv run python -m memsearch.backfill.cli source-sync antigravity --machine <machine>
```
